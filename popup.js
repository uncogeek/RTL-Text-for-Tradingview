...
function updateTextareaStyle() {
  ...
  // Set text direction and margin
  textarea.style.direction = settings.rtlMode ? 'rtl' : 'ltr';
  textarea.style.textAlign = settings.rtlMode ? 'right' : 'left';
  textarea.style.marginRight = settings.rtlMode ? '0' : '-20px';
  textarea.style.marginLeft = settings.rtlMode ? '0' : '0';
...
function generateImage(callback) {
  ...
  try {
    ...
    const startX = settings.rtlMode 
      ? canvas.width - settings.padding - borderWidth 
      : settings.padding + borderWidth;
